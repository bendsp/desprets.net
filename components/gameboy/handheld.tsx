...
import type { Control } from "./scene";
...
export function Handheld({ iframe, initialPath, open, power, bright, focus, reduced, pressed, onControl, onFold, onPower, onFocus }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    lidAngle.current = reduced ? target : MathUtils.damp(lidAngle.current, target, 6, Math.min(delta, .05));
    if (lid.current) lid.current.rotation.x = lidAngle.current;
    if (Math.abs(lidAngle.current - target) > .0001) invalidate();
  });
  ...
  const pressY = (control: Control) => pressed === control ? -.035 : 0;
  const button = (label: "A" | "B", control: Control, x: number, z: number) => <group key={control} position={[x, 0, z]}>
    ...
    <Disc position={[0, .219 + pressY(control), 0]} radius={.22} depth={.089} color={pressed === control ? "#505969" : "#303944"} onClick={() => onControl(control)} />
    <Label text={label} position={[0, .268 + pressY(control), 0]} width={.135} height={.145} rotation={[-Math.PI / 2, 0, 0]} color="#8d96a3" />
  </group>;
  ...
  return <group>
    ...
    <group position={[-.93, pressed && ["up", "down", "left", "right"].includes(pressed) ? .192 : .224, -.22]}>
    ...
    {([['SELECT', 'select', -.34], ['START', 'start', .34]] as const).map(([label, control, x]) => <group key={control}>
      ...
      <Disc position={[x, .22 + pressY(control), 1.07]} radius={.112} depth={.063} color="#333e4c" onClick={() => onControl(control)} />
    ...
    <Disc position={[0, .213 + pressY("light"), -.96]} radius={.108} depth={.035} color="#66717f" onClick={() => onControl("light")} />
    <Label text="☀" position={[0, .236 + pressY("light"), -.96]} width={.12} height={.12} rotation={[-Math.PI / 2, 0, 0]} color="#bbc4cf" />
